...
import { canRegister, canLogin,showAboutUs } from './state';

let navbar_Links = [
    {name: 'Home', action: toggleHomePage,id:"home"},
    {name: 'Live', action: toggleLivePage ,id:"live"},
    {name: 'About Us', action: toggleShowAboutUs ,id:"about"},
    {name: 'Contact Us', action: toggleContactUs,id:"contact"},
]

function toggleHomePage(){
    console.log("Home");
}
function toggleLivePage(){
    console.log("Live");
}
...
<template>
  <div v-show="!canLogin || !canRegister" class=" absolute w-full bg-transparent flex place-items-center px-10 z-10 backdrop-blur-3xl" style="height: 60px;">
    ...
    <div class="links ml-auto mr-5 bg-white flex h-10 place-items-center [&>a]:w-[90px] [&>a]:h-full py-1 [&>a]:place-items-center [&>a]:bg-white [&>a]:text-center gap-2 px-1 rounded [&>a]:rounded">
        <a v-for="link in navbar_Links" :key="link.name" @click="link.action" :href="link.link" :id="link.id" class=" cursor-pointer text-[#1a1a1a] font-semibold hover:bg-[#d9d9d985] text-[15px] transition-all flex justify-center hover:w-[110px]" style="font-family:'Roboto'">            {{ link.name }}
        </a>
