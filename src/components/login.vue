...
function togglePassword() {
  input_ps_type.value =
    input_ps_type.value === "password" ? "text" : "password";
}
function unLogin() {
  if (terms.value) {
    toggleterms(false);
...
<template>
  <div
    class="p-10 w-screen h-screen bg-[#4b4b4b1a] flex flex-col justify-center mt-0 backdrop-blur-lg absolute z-20"
    v-show="canLogin"
  >
    <div
      class="login w-full h-full bg-gray-100 rounded-2xl flex p-3 gap-3 shadow-2xl"
    >
      <div
        id="sec1log"
        class="sec1 w-1/2 gradient-background rounded-2xl"
      ...
      >
        <div class="sec1-pattern w-full h-full p-20 flex flex-col pt-10 pb-10">
          ...
          <h1
            class="mt-5 text-white flex place-items-center text-[50px] font-[800]"
            style="font-family: 'Roboto'; line-height: 50px"
          >
            Welcome To Habesha live
          </h1>
          <h3 class="text-white mt-3">
            wlecome back to your habesha live account sign back in and continue
            your joureny
          </h3>
          <Card
            v-show="!terms"
          ...
          >
            <CardContent
              class="flex place-items-center justify-start p-2 gap-3"
            >
              <IconEyeQuestion stroke="{2}" color="white" class="w-14 h-14" />
              <h1 class="flex place-items-center text-sm text-white">
                Want to see where your data goes
              </h1>
            </CardContent>
          </Card>
      ...
      <div
        class="sec2 w-1/2 h-full flex-col p-20 py-20 gap-6 [&>Label]:font-['Onest']"
      >
        ...
        <Button
          ...
          >Login</Button
        >
        <h1 class="w-full text-center mt-3" style="font-family: 'Onest'">
          Dont have an account
          <Span
            @click="register"
            class="text-[#e22abf] cursor-pointer underline"
            >Get started</Span
          >
        </h1>
      </div>
...
.sec1-pattern {
  ...
  background-image: linear-gradient(
      135deg,
      rgba(69, 78, 247, 0.23) 25%,
      transparent 25%
    ),
    linear-gradient(225deg, rgba(69, 78, 247, 0.23) 25%, transparent 25%),
    linear-gradient(45deg, rgba(69, 78, 247, 0.23) 25%, transparent 25%),
    linear-gradient(
      315deg,
      rgba(69, 78, 247, 0.23) 25%,
      rgba(250, 145, 145, 0) 25%
    );
